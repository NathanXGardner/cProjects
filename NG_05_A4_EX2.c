/* Nathan Gardner
   ...
   character length is met minus one (to account for the null
	terminator). The user's array of characters is then compared to the
	specific pointer of the capital names' array using the string comparison
	function strcmp, and then a value of 0 (false) is returned if they are
	equal. This function takes the number of questions the
	user answered, the number of questions the user correctly answered, and
	the percentage of questions the user correctly answered, and prints these
...
int main()
{
   ...
   int sentinel = 1;
	...
	printf("\nYou answered %d correctly out of %d for %f percent correct.",
			  correct_answers, total_questions, percent);
